...
if "lxplus" in NODENAME:
    LOCAL_DIR = "/eos/user/f/fernanpe/mds_analysis_240521/"
...
CUT_VALUES = {
    ...
    "ropt_low" : {
        ...
        # "MIN_CSC_DNN": 0,
        # # "MIN_DT_DNN": 0,
        # Optimized with IT S/rt[B]. S=1187.8, B=336, S/rt[B]=64.8
        "MIN_CSC_TIME": -5.0,
        "MAX_CSC_TIME": 12.5,
        "MAX_CSC_TSPREAD": 20.0,
        "MAX_RPC_BX": 0,
        "MIN_RPC_HITS": 1,
        "MAX_CSC_JET": 10,
        "MAX_DT_JET": 10, #! redo this
        "MAX_CSC_MUON": 10,# 45,
        "MAX_DT_MUON": 10, # 0, #! redo this
        "MAX_ME1": 0,  # 3,  # HLT requires this to be 0
        "MAX_MB1": 0,
        "HALO_CUTOFF": 0.0, # applied to dt phi #! redo this
        "MIN_DPHI": 1.8,
        "MIN_DETA": 0,
        "MAX_DETA": 4,
        # "MIN_CSC_DNN": 0,
        # "MIN_CSC_DNN": 0,  # bkgMC: S=, B=, S/rt[B]=
        "MIN_CSC_DNN": 0.96,  # pedro's value, bkgMC_plusBeamHalo: S=, B=, S/rt[B]=
        # "MIN_CSC_DNN": 0,  # bkgOOTData: S=, B=, S/rt[B]=
    },
    "ropt_high" : {
        "MIN_CSC_TIME": -5.0,
        "MAX_CSC_TIME": 12.5,
        ...
        # "MIN_CSC_DNN": 0,
        "MIN_CSC_DNN": 0.96,  # pedro's value, bkgMC_plusBeamHalo: S=, B=, S/rt[B]=
        # "MIN_CSC_DNN": 0.96137905,  # bkgMC
        # "MIN_CSC_DNN": 0.96137905,  # bkgMC_plusBeamHalo
        # "MIN_CSC_DNN": 0.96137905,  # bkgOOTData
        # "MIN_DT_DNN": 0,
    },
}
    ...
    for iopt in range(N_ITERATIONS):
        ...
        for key, rdf in rdfs.items():
            ...
            for cut in CUTS:  #! Can I run this in a subprocess?
                ...
                if "CSC jet req" in cut: # any of the clusters matched to a jet
                    rdf = rdf.Define(f"passJetMatch", f"{C}0CutFlag && ({C}JetVetoPt >= {MAX_CSC_JET})")
                    rdf = rdf.Redefine(f"evtCutFlag", f"evtCutFlag && (reduce(passJetMatch.begin(), passJetMatch.end()) > 0)")
                ...
                if "DNN $<$" in cut:
                    rdf = rdf.Define(f"InvertDNN", f"{C}0CutFlag && "
                        + f"( Take({C}DNN_{DNN_VERSION},nCscRechitClusters) <= {MIN_CSC_DNN} )"
                    )
                    rdf = rdf.Redefine(f"evtCutFlag", f"evtCutFlag && (reduce(InvertDNN.begin(), InvertDNN.end()) > 0)")
