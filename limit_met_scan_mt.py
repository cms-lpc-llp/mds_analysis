...
for min_met in range(sys.argv[-2], sys.argv[-1]+dmet, dmet):
    ...
    if sum([ms.count() < 3 for ms in mss]):
        print(f"LESS THAN 3 EVENTS FOUND -- EXITING ({min_met=})")
        break
    ...
    for ims, ms in enumerate(mss):
        ...
        _has_data = np.array([
            (np.sum((df_cond[idf])&(~sz_cond[isz]))>0)*
            (np.sum((~df_cond[idf])&(~sz_cond[isz]))>0)*
            (np.sum((~df_cond[idf])&(sz_cond[isz]))>0) for idf in range(nth_df) for isz in range(nth_sz)])
    ...
    for ct in ctaus:
        ...
        if len(abcd_sig) == 0 or len(abcd_obs) == 0 or (np.asarray(abcd_sig) == 0).any() or (np.asarray(abcd_obs) == 0).any():
            continue
        ...
        fdc = f"datacard_ct{ct:0>8.0f}_met{min_met:0>3.0f}.txt"
