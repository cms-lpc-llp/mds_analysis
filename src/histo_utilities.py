...
def make_ratio_pEff(h_list_in,
  ...
  h_list[0].GetPaintedGraph().GetHistogram().GetXaxis().SetLimits(h_list_in[0].GetCopyTotalHisto().GetXaxis().GetXmin(),
                                                                  h_list_in[0].GetCopyTotalHisto().GetXaxis().GetXmax())
  ...
  for j in range(band.GetXaxis().GetNbins()):
    ...
    if h_list[0].GetEfficiency(j + 1) == 0:
      ...
      band.SetBinError(j + 1,
                       h_list_in[ratio_index].GetEfficiencyErrorUp(j + 1) / h_list_in[ratio_index].GetEfficiency(j + 1))
  ...
  for i, h in enumerate(h_list):
    if i == ratio_index:
      ...
      for j in range(nom.GetXaxis().GetNbins() + 1):
        ...
        denom.SetBinError(
            j + 1, max(h_list[ratio_index].GetEfficiencyErrorLow(j + 1),
  ...
  for i, h in enumerate(h_list):
    if i == 0:
      ...
      if len(text) > 0:
        ...
        l.SetTextSize(0.045)
        if logy:
          l.DrawLatex((h.GetXaxis().GetXmax() - h.GetXaxis().GetXmin()) * 0.1 + h.GetXaxis().GetXmin(),
                      h.GetMaximum() / 10, text)
        else:
          l.DrawLatex((h.GetXaxis().GetXmax() - h.GetXaxis().GetXmin()) * 0.1 + h.GetXaxis().GetXmin(),
                      h.GetMaximum() * 0.8, text)
    else:
      h.DrawCopy(draw_opt[i] + "same")
      # h.DrawCopy("E1 same")
...
def binning2d(histo2D, min_event,
              direc):  #returns an appropriate binning in x/y direction with at least min_event points in each binning
  bins = [1]
  i = 0
  if direc == 'x':
...
def histo2D_projectionFit(histo2D, bins, gaus_thr, rebin,
                          direc):  #given the binning, plot the scale and resolution of the projection of 2D map
  proj = []
  if direc == 'x':
