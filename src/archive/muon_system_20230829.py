    ...
    def filter(self, sel, system="evt", has_clusters=True):
        ...
        if system != "evt":
            ...
            if has_clusters:
                ...
                self.filter(
                    (self.ms_read["nCscRechitClusters"] > 0) & (self.ms_read["nDtRechitClusters"] > 0), system="evt"
                )
        ...
        def _delta_kins(csc_phi, csc_eta, dt_phi, dt_eta, met_phi, sel_csc, sel_dt, tag):
            ...
            for i in range(len(csc_eta)):
                ...
                if sum([cd_id == -1 for cd_id in [d0, d1, c0, c1]]) > 2:
                    raise NotImplementedError("More than 2 clusters found!")
...
# class MuonSystemAwkward: #old/deprecated version
#     """Handler for working with muon system ntuples using an Awkward arrays"""
...
class MuonSystemRDF:
    ...
    def get(self, key):
        ...
        if ctype == "int":
            ...
            return self.rdf.Take[VecOps.RVec[int]](key)
        elif ctype == "RVec<float>":
            return self.rdf.Take[VecOps.RVec[float]](key)
        elif ctype == "RVec<bool>":
            return self.rdf.Take[VecOps.RVec[bool]](key)
        else:
    ...
    def find_csc_trigger(self, implicit=True):
        ...
        self_out = self.Define(
            "idxCscRechitClusterTrigger",
            """
        int iTrig = -1;
    ...
    def time_cut(self, time="it", system="cscdt", implicit=True):
        if time == "oot":
            ...
            if "csc" in system:
                self_out = self.Filter(
                    "-5 < cscRechitClusterTimeWeighted && cscRechitClusterTimeWeighted < 12.5 && cscRechitClusterTimeSpreadWeightedAll < 20",
                    system="csc",
                    implicit=implicit,
                )
            if "dt" in system:
                self_out = self.Filter(
                    "(dtRechitCluster_match_RPCBx_dPhi0p5 == 0) && (dtRechitCluster_match_RPChits_dPhi0p5 > 0)",
                    system="dt",
                    implicit=implicit,
                )
        return self_out
