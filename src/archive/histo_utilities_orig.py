...
def create_TH1D(
    ...
    if h2clone == None:
        ...
        if binning[0] is None:
            bin_w = 4 * (np.percentile(x, 75) - np.percentile(x, 25)) / (len(x)) ** (1.0 / 3.0)
            if bin_w == 0:
                bin_w = 0.5 * np.std(x)
            if bin_w == 0:
                bin_w = 1
...
def create_prof1D(x, y, name="h", title=None, binning=[None, None, None], h2clone=None, axis_title=["", ""], opt=""):
    ...
    for i in range(1, binning[0] + 1):
        xl = h.GetBinCenter(i) - h.GetBinWidth(i) / 2.0
        xu = h.GetBinCenter(i) + h.GetBinWidth(i) / 2.0
...
def create_TH2D(
    ...
    if sample.shape[0] == 0:
        ...
        if binning[0] is None:
            bin_w = (
                4
                * (np.percentile(sample[:, 0], 75) - np.percentile(sample[:, 0], 25))
                / (len(sample[:, 0])) ** (1.0 / 3.0)
            )
            if bin_w == 0:
        ...
        if binning[3] == None:
            bin_w = (
                4
                * (np.percentile(sample[:, 1], 75) - np.percentile(sample[:, 1], 25))
                / (len(sample[:, 1])) ** (1.0 / 3.0)
            )
            if bin_w == 0:
...
def make_ratio_pEff(
    ...
    h_list[0].GetPaintedGraph().GetHistogram().GetXaxis().SetLimits(
        h_list_in[0].GetCopyTotalHisto().GetXaxis().GetXmin(), h_list_in[0].GetCopyTotalHisto().GetXaxis().GetXmax()
    )
    ...
    if logy:
        h_list[0].GetPaintedGraph().GetHistogram().GetYaxis().SetRangeUser(
            10e-7, 150 * max(map(lambda x: x.GetPaintedGraph().GetHistogram().GetMaximum(), h_list))
        )
    ...
    for j in range(band.GetXaxis().GetNbins()):
        ...
        if h_list[0].GetEfficiency(j + 1) == 0:
            ...
            band.SetBinError(
                j + 1, h_list_in[ratio_index].GetEfficiencyErrorUp(j + 1) / h_list_in[ratio_index].GetEfficiency(j + 1)
            )
        ...
        if i == ratio_index:
            ...
            for j in range(nom.GetXaxis().GetNbins() + 1):
                ...
                denom.SetBinError(
                    j + 1,
                    max(
                        h_list[ratio_index].GetEfficiencyErrorLow(j + 1),
                        h_list[ratio_index].GetEfficiencyErrorUp(j + 1),
                    ),
                )
            nom.Divide(denom)
...
def make_ratio_plot(
    ...
    for i, h in enumerate(h_list):
        if i == 0:
            ...
            if len(text) > 0:
                ...
                if logy:
                    l.DrawLatex(
                        (h.GetXaxis().GetXmax() - h.GetXaxis().GetXmin()) * 0.1 + h.GetXaxis().GetXmin(),
                        h.GetMaximum() / 10,
                        text,
                    )
                else:
    ...
    for j in range(band.GetXaxis().GetNbins()):
        ...
        if h_list[ratio_index].GetBinContent(j + 1) == 0:
            ...
            band.SetBinError(
                j + 1, h_list_in[ratio_index].GetBinError(j + 1) / h_list_in[ratio_index].GetBinContent(j + 1)
            )
            # print(j, h_list_in[0].GetBinError(j+1)/h_list_in[0].GetBinContent(j+1))
    band.SetFillColor(h_list_in[ratio_index].GetLineColor())
        ...
        if i == ratio_index:
            ...
            for j in range(h.GetXaxis().GetNbins()):
                if h_list[i].GetBinContent(j + 1) == 0:
                    ...
                    h.SetBinError(
                        j + 1,
                        h_list_in[i].GetBinError(j + 1) / h_list_in[i].GetBinContent(j + 1) * h.GetBinContent(j + 1),
                    )
...
def binning2d(
    histo2D, min_event, direc
):  # returns an appropriate binning in x/y direction with at least min_event points in each binning
    bins = [1]
    i = 0
    if direc == "x":
...
def histo2D_projectionFit(
    histo2D, bins, gaus_thr, rebin, direc
):  # given the binning, plot the scale and resolution of the projection of 2D map
    proj = []
    if direc == "x":
...
def create_TGraph(x, y, ex=[], ey=[], axis_title=["", ""]):
    ...
    if len(ex) == 0:
        ...
        gr = rt.TGraphErrors(len(x), x, y, ex, ey)
    if len(axis_title) == 2:
