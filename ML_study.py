"""Study for developing cuts on the muon system."""
...
if __name__ == "__main__":
    ...
    ########
    print("LOADING DATA")
    # SEEMS TO DO WORSE WHEN COMBINED? test individual aucs too !!
    # df = pd.concat([pd.read_csv(f"{in_data_dir}csc{s}_processed_{t}_{stat}.csv") for s in ("csc","dt") for t in ("mc","r3")], ignore_index=True)
    pair = 'csccsc' # 'cscdt'
    print(pair)
    df = pd.concat([pd.read_csv(f"{in_data_dir}{pair}_processed_{t}_{stat}.csv") for t in ("mc","r3")], ignore_index=True)
    print(df)

    X, y = df[['met','dR','dEta','dPhi', 'isCscCsc']], df['isMC']
    ...
    #fig, axs = plt.subplots(, len(names), figsize=((len(names)) * 3, 6 * 3))
    cmap = "RdBu"

    for iclf, (name, clf) in enumerate(zip(names, classifiers)):
        print(f"({iclf+1:,}/{len(classifiers):,}) - {name=}")
        clf = make_pipeline(StandardScaler(), PCA(0.99), clf)
