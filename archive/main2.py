...
from src.helper_functions import getRecoTime, find_nearest, deltaPhi, deltaR, lor, land, lxor, lnot, canvas, weight_calc
...
if __name__ == "__main__":
    ...
    ms["csc_gLLP_decay"] = land(
        ms["gLLP_abs_eta"] < 2.4,
        ms["gLLP_decay_vertex_abs_r"] < 695.5,
        ms["gLLP_decay_vertex_z"] < 1100,
        ms["gLLP_decay_vertex_abs_z"] > 661,
    )
    ms["dt_gLLP_decay"] = land(
        ms["gLLP_decay_vertex_abs_z"] < 661, ms["gLLP_decay_vertex_abs_r"] > 380, ms["gLLP_decay_vertex_r"] < 738
    )
    ...
    ms["csc_pass_muon_veto"] = lnot(
        lor(
            ...
            ms["cscRechitClusterNRechitChamberMinus11"]
            + ms["cscRechitClusterNRechitChamberMinus12"]
            + ms["cscRechitClusterNRechitChamberPlus11"]
            + ms["cscRechitClusterNRechitChamberPlus12"]
            > 0,
    ...
    for bins, stat in zip([(201, -80, 80), (21, -10, 10)], ["full", "zoom"]):
        ...
        hdt_sig = ms.Histo1D(
            "dtRechitCluster_match_RPCBx_dPhi0p5[dt_pass_jet_muon_veto]", bins=bins, c=std_color_list[4]
        )
        hdt_bkg = ms.Histo1D(
            "dtRechitCluster_match_RPCBx_dPhi0p5[dt_fail_jet_muon_veto]", bins=bins, c=std_color_list[5]
        )
    ...
    for bins, stat in zip([(201, -80, 80), (21, -10, 10)], ["full", "zoom"]):
        ...
        hcsc_sig = ms.Histo1D(
            "cscRechitClusterTimeWeighted[csc_pass_jet_muon_veto_cscdt_cscL1]", bins=bins, c=std_color_list[1]
        )
        hcsc_bkg = ms.Histo1D(
            "cscRechitClusterTimeWeighted[csc_fail_jet_muon_veto_cscdt_cscL1]", bins=bins, c=std_color_list[2]
        )
        hdt_all = ms.Histo1D("dtRechitCluster_match_RPCBx_dPhi0p5[evt_cscdt_cscL1]", bins=bins, c=std_color_list[3])
        hdt_sig = ms.Histo1D(
            "dtRechitCluster_match_RPCBx_dPhi0p5[dt_pass_jet_muon_veto_cscdt_cscL1]", bins=bins, c=std_color_list[4]
        )
        hdt_bkg = ms.Histo1D(
            "dtRechitCluster_match_RPCBx_dPhi0p5[dt_fail_jet_muon_veto_cscdt_cscL1]", bins=bins, c=std_color_list[5]
        )
    ...
    evt_pass_csc_jet_muon_dt_jet_muon = land(
        sel_1csc1dt, ak.sum(csc_pass_jet_muon, axis=1) == 1, ak.sum(dt_pass_jet_muon, axis=1) == 1
    )
    ...
    evt_pass_csc_jet_muon_int_dt_jet_muon_int = land(
        sel_1csc1dt, ak.sum(csc_pass_jet_muon_int, axis=1) == 1, ak.sum(dt_pass_jet_muon_int, axis=1) == 1
    )
    ...
    evt_pass_csc_jet_muon_int_dt_jet_muon_int_HLT = land(
        evt_pass_csc_jet_muon_int_dt_jet_muon_int, ms["HLTDecision"][:, 569]
    )
    ...
    if is_mc:
        ...
        for met in [False, True]:
            ...
            data = ms.arrs
            sel_dt = land(
                data["gLLP_decay_vertex_abs_z"] < 661,
                data["gLLP_decay_vertex_abs_r"] > 380,
                data["gLLP_decay_vertex_r"] < 738,
            )

            sel_csc = land(
                data["gLLP_abs_eta"] < 2.4,
                data["gLLP_decay_vertex_abs_r"] < 695.5,
                data["gLLP_decay_vertex_z"] < 1100,
                data["gLLP_decay_vertex_abs_z"] > 661,
            )

            me11 = land(
                data["gLLP_abs_eta"] < 2.4,
                data["gLLP_decay_vertex_abs_r"] < 270,
                data["gLLP_decay_vertex_abs_z"] < 661,
                    ...
                    for ct in plot_ctaus:
                        if ct in ctaus:
                            ...
                            for l, ct0 in enumerate(ct_list):
                                ...
                                weight_ctau = weight_calc(
                                    gLLP_ctau, ctf / 10, int(ct0) / 10
