...
bad_cols = (
    ...
    "dtRechitCluster2ZLep1LooseIso",
    "dtRechitCluster22ZLep1LooseIso",
    "dtRechitClusterJetVetoPtJESDown",
    "dtRechitClusterTightJetVetoPtJESDown",
)
...
def update_cols(targ_rdf, cut: str, cut_col: str, col_type: str, n_cluster: int = 0):
    ...
    for col in targ_rdf.GetColumnNames():
        ...
        if (
            str(col)[: len(cut_prefix)] == cut_prefix
            and isvec
            and ctype in ("Float_t", "Int_t", "Bool_t")
            and col not in bad_cols
        ):
...
def _cbid_cut(csc_eta, csc_avgstation, csc_nstation):
    ...
    return (n > 1) * (e < 1.9) + (n == 1) * (
        (a == 4) * (e < 1.8) + (a == 3) * (e < 1.6) + (a == 2) * (e < 1.6) + (a == 1) * (e < 1.1)
    )
...
def apply_csc_eta_cut(targ_rdf, max_eta: float = 2, cut_col: str = "temp_cut_col", n_cluster: int = 0):
    targ_rdf = update_cols(
        targ_rdf, f"Numba::_csc_eta_cut(cscRechitClusterEta, {max_eta})", cut_col, "csc", n_cluster=2
    )
    return targ_rdf  # .Filter('nCscRechitClusters + nDtRechitClusters > 0')
...
def apply_time_spread_cut(
    targ_rdf, max_spread: float = 20, col_type: str = "csc", cut_col: str = "temp_cut_col", n_cluster: int = 0
):
    # targ_rdf = update_cols(targ_rdf, f'Numba::_time_spread_cut({col_type}RechitClusterTimeSpread, {max_spread})', cut_col,
...
def apply_oot_cut(
    targ_rdf, max_t: float = -12.5, col_type: str = "csc", cut_col: str = "temp_cut_col", n_cluster: int = 0
):
    # targ_rdf = update_cols(targ_rdf, f'Numba::_oot_cut({col_type}RechitClusterTime, {max_t})', cut_col, col_type, n_cluster=2)
    return targ_rdf  # .Filter('nCscRechitClusters + nDtRechitClusters > 0')
...
if __name__ == "__main__":
    ...
    fpath = "data/raw/GBC_0jet_DisplacedJet-EXOCSCCluster_Run2022EFG-PromptReco-v2_goodLumi.root"
    ...
    # ! Always include the max size 100 cut! This blinds the data
    # TODO: Do this before any analysis and save it as a separate data file!!!
    # rdfs = {'raw': {'rdf': rdf, 'line_color': rt.kBlack}}  # Blinded data
    rdfs = {
        "raw": {"rdf": apply_size_cut(apply_size_cut(rdf, 0, 100, "csc"), 0, 100, "dt"), "line_color": rt.kBlack}
    }  # Blinded data
    # rdfs = {'raw': {'rdf': apply_size_cut(rdf, 100), 'line_color': rt.kBlack}}  # Signal
    ...
    for k, rdf_info in rdfs.items():
        ...
        rdf_info["rdf"] = rdf_info["rdf"].Define(
            "cscRechitCluster_match_gLLP_decay_abs_z", "abs(cscRechitCluster_match_gLLP_decay_z)"
        )
    ...
    c1.cd(1).SetLogz()
    c1.cd(1).SetGrid()
    h1 = rdfs["raw"]["rdf"].Histo2D(
        ("", ";csc;dt", 7, -0.5, 6.5, 7, -0.5, 6.5), "nCscRechitClusters", "nDtRechitClusters"
    )
    h1.Draw("coltext")
    ...
    template = (
        (
            ...
            "delta_metPhi_cscRechitClusterPhi;\phi_{ME_{t}} - \phi_{CSC}",
            100,
            -np.pi,
            np.pi,
        ),
    ...
    for it, template in enumerate(templates):
        ...
        ccs = [rt.kBlack, rt.kYellow - 2, rt.kRed, rt.kBlue]
        tts = ["Raw", "50 < MET", "All Cuts, 50 < Met", "OOT (t < -12.5)"]
        for ih, (hh, cc, tt) in enumerate(zip(hhs, ccs, tts)):
            hh.SetLineColor(cc)
    ...
    templates = [
        ...
        (
            ("cscRechitClusterAvgStation10", "cscRechitClusterAvgStation10", 6, -0.5, 5.5),
    ...
    template = (
        ("csc_corr", "csc_corr;Small Cluster Size;Big Cluster Size", 100, 0, 1000, 100, 0, 1000),
        "csc_size0",
        "csc_size1",
    )
    ...
    # hhs = [h0, h1, h2, h3]
    # ccs = [rt.kBlack, rt.kYellow - 2, rt.kGreen, rt.kRed, rt.kMagenta]
    tts = ["Raw", "met50", "All Cuts, 50 < Met"]  # '^^ + matched'
