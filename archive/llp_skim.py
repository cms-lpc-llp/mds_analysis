...
for iev, ev in enumerate(tt):
  ...
  for eta, nstation10, avgstation10 in zip(ev.cscRechitClusterEta, ev.cscRechitClusterNStation10,
                                           ev.cscRechitClusterAvgStation10):
  ...
  for i, (csc_eta, csc_phi) in enumerate(zip(ev.cscRechitClusterEta, ev.cscRechitClusterPhi)):
    ...
    for j, (lep_eta, lep_phi, lep_pdg) in enumerate(zip(ev.lepEta, ev.lepPhi, ev.lepPdgId)):
      if abs(lep_pdg) == 13:
        pass_CM_dR_idxs[i] = min(pass_CM_dR_idxs[i], np.sqrt((csc_eta - lep_eta)**2 + (csc_phi - lep_phi)**2))
  ...
  pass_cluster_rechits_ME11_ME12 = [
      (M11 + M12 + P11 + P12) == 0
      for M11, M12, P11, P12 in zip(ev.cscRechitClusterNRechitChamberMinus11, ev.cscRechitClusterNRechitChamberMinus12,
                                    ev.cscRechitClusterNRechitChamberPlus11, ev.cscRechitClusterNRechitChamberPlus12)
  ]
  ...
  evs_dict['rawM'] = build_ntuple_row(ev,
                                      cut_val=pass_csc_match,
                                      col='cscRechitCluster',
                                      tdict=evs_dict['rawM'],
                                      more_cols_dict=more_cols_dict)
