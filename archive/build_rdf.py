...
def apply_csc_eta_cut(targ_rdf, max_eta: float = 2, cut_col: str = "temp_cut_col", n_cluster: int = 0):
    targ_rdf = update_cols(
        targ_rdf, f"Numba::_csc_eta_cut(cscRechitClusterEta, {max_eta})", cut_col, "csc", n_cluster=n_cluster
    )
    return targ_rdf
...
if __name__ == "__main__":
    ...
    for k, rdf_info in rdfs.items():
        ...
        rdf_info["rdf"] = rdf_info["rdf"].Define(
            "deltaE", "Numba::add_delta_e(gHiggsE, cscRechitCluster_match_gLLP_e, jetE)"
        )
        ...
        rdf_info["rdf"] = rdf_info["rdf"].Define(
            "cscRechitCluster_match_gLLP_decay_abs_r", "abs(cscRechitCluster_match_gLLP_decay_r)"
        )
