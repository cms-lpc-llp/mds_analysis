            ...
            for cut in CUTS:  #! Can I run this in a subprocess?
                ...
                if "HLT" in cut:
                    ...
                    if "mc" in key and "hlt" not in FN_MC:
                        # rdf = rdf.Redefine("evtCutFlag", "evtCutFlag && HLTDecision[566]") #!
                        # def hlt_cut(_r):
                        #     return _r.Redefine("evtCutFlag", "evtCutFlag && HLTDecision[566]")
                        # with Pool(2) as pool:
                        #     rdf = pool.apply(hlt_cut, rdf)
                        # continue
                        pass
                    elif "r3" in key and "hlt" not in FN_R3:
                        # 566 = CscCluster_Loose
                        # 569 = HLT_L1CSCCluster_DTCluster50
                        rdf = rdf.Redefine("evtCutFlag", "evtCutFlag && HLTDecision[566] && (runNum >= 360019)")
                        # rdf = rdf.Redefine("evtCutFlag", f"evtCutFlag && evtFlag && (runNum >= 360019)")
                        # def hlt_cut(_r):
                        #     return _r.Redefine("evtCutFlag", "evtCutFlag && HLTDecision[566]")
                        # with Pool(1) as pool:
                        #     rdf = pool.apply(hlt_cut, rdf)
                ...
                if "CSC0 largest" in cut:
                    ...
                    #! lisa attempt 1. excess of 0.1% with data, why?
                    # rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && !( {C}0Size == Max({C}Size*{C}0Flag*{C}0CutFlag) )")
                    # rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && ( {C}0Size == Max({C}Size*{C}0Flag*{C}0CutFlag) )")
                    # rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && !{C}0CutFlag")
                    #! lisa attempt 2, still excess
                    # rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && ( {C}0Size == ({C}Size[{C}0Flag && {C}0CutFlag])[0] )")
                    # rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && !{C}0CutFlag")
                    #! lisa attempt 3
                    rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && !( {C}Size == ({C}Size[{C}0Flag && {C}0CutFlag])[0] )")
                    rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && ( {C}Size == ({C}Size[{C}0Flag && {C}0CutFlag])[0] )")
                    # rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && !( {C}Size*({C}0Flag*{C}0CutFlag) == ({C}Size[{C}0Flag && {C}0CutFlag])[0] )")
                    # rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && ( {C}Size*({C}0Flag*{C}0CutFlag) == ({C}Size[{C}0Flag && {C}0CutFlag])[0] )")

                    # rdf = rdf.Filter("evtFlag", f"reduce(({C}0CutFlag && {C}1CutFlag).begin(), ({C}0CutFlag && {C}1CutFlag).end()) == 0") # make sure we have 0 overlap
                ...
                # **** #
                if "0 Leptons" in cut:
                    rdf = rdf.Redefine("evtCutFlag", f"evtCutFlag && (nLeptons==0)")
                ...
                if "ME1" in cut: # and OPT_CUT != "ME1":
                    if "CSC0" in cut or ("CSC0" not in cut and "CSC1" not in cut):
                        rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && ( {C}NHitME1 == 0 ) ")
                    if "CSC1" in cut or ("CSC0" not in cut and "CSC1" not in cut):
                        rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && ( {C}NHitME1 == {0} ) ") #!
                ...
                if "CSC" in cut and "jet veto" in cut: # and OPT_CUT != "CSC jet veto":
                    if "CSC0" in cut or ("CSC0" not in cut and "CSC1" not in cut):
                ...
                if "CSC" in cut and "muon veto" in cut: # and OPT_CUT != "CSC muon veto":
                    ...
                    if "CSC1" in cut or ("0" not in cut and "1" not in cut):
                        rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && "
                            # + f"!( !({C}MuonVetoGlobal == 0) && !({C}MuonVetoPt < {MAX_CSC_MUON}) )" #! is there a difference?
                            + f"!( ({C}MuonVetoGlobal > 0) && ({C}MuonVetoPt >= {MAX_CSC_MUON}) )"
                        )
                ...
                if "DNN $>$" in cut: 
                    MIN_CSC_DNN = float(cut.replace(",","").split(" ")[2]) #!
                    # rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && " #!
                ...
                if "1 CSC-CSC" in cut:
                    # rdf = rdf.Redefine("evtCutFlag", "evtCutFlag && (nCscRechitClusters == 2)") #!
                    # rdf = rdf.Redefine("evtCutFlag", "evtCutFlag && (nCscRechitClusters == 2) && (nDtRechitClusters == 0)") #! more like my code
                    ...
                    #     f"auto {C}01Flag_and = {C}0Flag && {C}1Flag;"
                    #     + f"return evtFlag && ( "
                    #     + f"(reduce({C}0Flag.begin(), {C}0Flag.end()) > 0) && "
                    #     + f"(reduce({C}1Flag.begin(), {C}1Flag.end()) > 0) && "
                    #     + f"(reduce({C}01Flag_and.begin(), {C}01Flag_and.end()) == 0) );"
                    # )
                    # rdf = rdf.Redefine(f"{C}0Flag", f"{C}0Flag && ( {C}0Size == Max({C}Size*{C}0Flag) )")
                    # rdf = rdf.Redefine(f"{C}1Flag", f"{C}1Flag && ( {C}1Size == Max({C}Size*({C}1Flag && !{C}0Flag)) )")
                    
                    
                    rdf = rdf.Redefine(f"{C}0Flag", f"{C}0Flag && ( {C}0Size == ({C}Size[{C}0Flag])[0] )")
                    rdf = rdf.Redefine(f"{C}1Flag", f"{C}1Flag && ( {C}1Size == ({C}Size[{C}1Flag])[0] )")
                    ...
                    # if "r3" in key:
                    #     rdf = rdf.Redefine("evtCutFlag", f"evtCutFlag && evtFlag && (runNum >= 360019)")
                    rdf = rdf.Redefine(f"{C}0CutFlag", f"{C}0CutFlag && {C}0Flag")
                    rdf = rdf.Redefine(f"{C}1CutFlag", f"{C}1CutFlag && {C}1Flag")
                    ...
                    # rdf = rdf.Define("tag_dPhi", f"abs( fmod(csc0Phi - csc1Phi + {PI}, {2*PI}) - {PI} )") #!
                    # rdf = rdf.Define("tag_dPhi", f"abs( fmod(csc1Phi - csc0Phi + {PI}, {2*PI}) - {PI} )") #!
                    # rdf = rdf.Define("tag_dPhi", 
                    #         f"auto _dphi = csc1Phi - csc0Phi + {PI}, tau = {2*PI};"+
                    #         # f"return abs( (_dphi/tau - floor(_dphi/tau))*tau - {PI} );")
                    #         f"_dphi = abs( (_dphi/tau - floor(_dphi/tau))*tau - {PI} );"+
                    #         f"auto _dphi2 = csc0Phi - csc1Phi + {PI};"+
                    #         f"_dphi2 = abs( (_dphi2/tau - floor(_dphi2/tau))*tau - {PI} );"+
                    #         f"return min(_dphi, _dphi2);")
                    # rdf = rdf.Define("tag_dPhi", 
                    #         f"auto _dphi = csc1Phi - csc0Phi + {PI}, tau = {2*PI};"+
                    #         f"return abs( (_dphi/tau - floor(_dphi/tau))*tau - {PI} );")
                    # rdf = rdf.Define("tag_dPhi", f"abs( fmod(abs(csc0Phi - csc1Phi) + {PI}, {2*PI}) - {PI} )") #!
                ...
                if "dPhi $<$" in cut:
                    MAX_DPHI = float(cut.split(" ")[2])
                    rdf = rdf.Redefine("evtCutFlag", f"evtCutFlag && (tag_dPhi < {MAX_DPHI})")
                elif "dPhi" in cut:
                    MIN_DPHI = float(cut.split(" ")[2]) if ">" in cut else MIN_DPHI
                ...
                # print(rdf.Filter(f"reduce({C}1Flag.begin(), {C}1Flag.end()) > 0").Sum("weight").GetValue())
                # print(rdf.Filter(f"reduce({C}1Flag.begin(), {C}1Flag.end()) > 1").Sum("weight").GetValue())
                ...
                if PRINT_CUTFLOW:
                    ...
                    e_cum_eff = f"{100*ec/ec0:.3g}" if cc0 != cc00 else "--"
                    e_cut_eff = f"{100*ecc/ec0:.3g}" if ccc0 != cc00 else "--"
                    c0_cum_eff = f"{100*cc0/cc00:.3g}" if cc0 != cc00 else "--"
                    c0_cut_eff = f"{100*ccc0/cc00:.3g}" if ccc0 != cc00 else "--"
                    c1_cum_eff = f"{100*cc1/cc10:.3g}" if cc1 != cc10 else "--"
                    c1_cut_eff = f"{100*ccc1/cc10:.3g}" if ccc1 != cc10 else "--"
                    d_cum_eff = f"{100*dc/dc0:.3g}" if dc != dc0 else "--"
                    d_cut_eff = f"{100*dcc/dc0:.3g}" if dcc != dc0 else "--"

                    print(f"    {cut.replace('_',' ')} & "+
                        f"{ec:,.2f} & {e_cut_eff} & {e_cum_eff} & "+
