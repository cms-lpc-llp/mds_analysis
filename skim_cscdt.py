...
if __name__ == '__main__':
        ...
        for cut in CUTS:
            ...
            if 'jet veto' in cut:
                if 'high MET' in CUTS:
                    ...
                    rdf = rdf.Redefine(f'{C}CutFlag', f'{C}CutFlag && ( ({C}JetVetoLooseId == 0) || ({C}JetVetoPt < 65) )')
                    rdf = rdf.Redefine(f'{D}CutFlag', f'{D}CutFlag && ( ({D}JetVetoLooseId == 0) || ({D}JetVetoPt < 180) )')
            
            if 'muon veto' in cut:
                rdf = rdf.Redefine(f'{C}CutFlag', f'{C}CutFlag && ( ({C}MuonVetoGlobal == 0) || ({C}MuonVetoPt < 60) )')
                rdf = rdf.Redefine(f'{D}CutFlag', f'{D}CutFlag && ( ({D}MuonVetoLooseId == 0) || ({D}MuonVetoPt < 150) )')
            ...
            if 'DNN' in cut:
                ...
                if 'high MET' in CUTS:
                    ...
                    rdf = rdf.Redefine(f'{C}CutFlag', f'{C}CutFlag && ( Take({C}DNN,nCscRechitClusters) > 0.68 )')
                    rdf = rdf.Redefine(f'{D}CutFlag', f'{D}CutFlag && ( Take({D}DNN,nDtRechitClusters) > 0.29 )')
    ...
    for key, rdf in rdfs.items():
        ...
        rdf = rdf.Snapshot('MuonSystem_flat', f'data/processed/{key}_cscdt_rdf.root', columns_out)
        rdfs[key] = rdf
        ...
        for xx in ('met', 'cscSize', 'cscR', 'cscEta', 'cscPhi', 'dtSize', 'dtR', 'dtEta', 'dtPhi', 'tag_dEta','tag_dPhi','tag_dR'):
            ...
            hh.Draw()
            canvas.Draw()
            canvas.Print(f'{OUT_DIR}/{key}_cscdt_{xx}.png')
    print('')
